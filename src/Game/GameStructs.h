...
// Forward declaration for OpenGL type used in BITMAP_t
#ifndef APIENTRY
#include <windows.h>
#endif
#include <GL/gl.h>

// Constant values (from Defines.h of the injection client reference):
//   MAX_BITMAP_FILE_NAME = 32
//   MAX_SPECIAL_OPTION   = 9
//   MAX_CLASS            = 4
//   MAX_RESISTANCE       = 4
//   MAX_SERVER_LO        = 20
...
struct ITEM /* Struct Size = 68 */
{
    ...
    /*+36*/ BYTE  SpecialNum;
    /*+37*/ BYTE  Special[9];      // MAX_SPECIAL_OPTION = 9
    /*+45*/ BYTE  SpecialValue[9]; // MAX_SPECIAL_OPTION = 9
    /*+56*/ DWORD Key;
    /*+60*/ BYTE  Unkown;
...
struct ITEM_ATTRIBUTE /* Struct Size = 64 */
{
    ...
    /*+51*/ // BYTEFILL (padding)
    /*+52*/ int  Money;
    /*+56*/ BYTE RequireClass[4];  // MAX_CLASS = 4
    /*+60*/ BYTE Resistance[4];    // MAX_RESISTANCE = 4
};
...
struct BITMAP_t
{
    char   FileName[32]; // MAX_BITMAP_FILE_NAME = 32
    float  Width;
    float  Height;
    char   Components;
...
struct SERVER_LIST_t
{
    ...
    BYTE     Number;
    bool     extServer;
    SERVER_t Server[20]; // MAX_SERVER_LO = 20
};
