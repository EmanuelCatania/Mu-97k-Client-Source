...
void __cdecl RenderTipText_stub(int sx, int sy, char* Text) {
    ...
    // Enable alpha test
    GL_SetBlendSrcOver('\x01');  // EnableAlphaTest(true)
    ...
    GL_DrawRect((float)textSize.cx + x + _DAT_00552540, y,
                 1.0f, (float)textSize.cy + _DAT_00552650);
    // Bottom border line (horizontal)
    GL_DrawRect(x, (float)textSize.cy + y + _DAT_00552540,
                 (float)textSize.cx + _DAT_00552650, 1.0f);

    // Semi-transparent black fill
    glColor4f(0.0f, 0.0f, 0.0f, 0.8f);  // 0x3f4ccccd ≈ 0.8
    GL_DrawRect((float)sx - _DAT_0055256c, (float)sy - _DAT_0055264c,
    ...
    switch (savedBlendType) {
        case 1:  GL_EnableLightMap();    break;  // EnableLightMap
